...
power_wide <- tibble(period_1 =  power_1$kly_kW,  period_2 = NA)
...
kly_e_out <-
    power_wide |>
    select(period_1) |>
    mutate(MW = period_1 / 1000, .keep = "unused") |>
    arrange(desc(MW)) |>
    mutate(duration = seq(1,n()))
...
kly_e_out |>
    ggplot(aes(x = duration, y = MW)) +
    labs(
        title = "Kalayan 2 - Yearly Output Power (theoretical)",
    ...
    annotate("rect", 
             ...
             xmax = dur_max_power, 
             ymin = 0, 
             ymax = installed_capacity, alpha = 0.2, color = "green", fill="green") +
    annotate("rect", 
             xmin = 8760 - dur_null_power, 
             xmax = 8760, 
             ymin = 0, 
             ymax = installed_capacity, alpha = 0.2, color = "red", fill="red") +
