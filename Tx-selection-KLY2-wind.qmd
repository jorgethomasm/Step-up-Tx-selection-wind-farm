...
rdxklywind_xts <- xts(rdxklywind[, 2:ncol(rdxklywind)], as.POSIXct(rdxklywind$datetime))

ends <- endpoints(rdxklywind_xts, "hours")
ends <- ends + 1
ends[length(ends)] <- ends[length(ends)] - 1

rdxklywind_hh <- period.apply(rdxklywind_xts, ends, mean)

wind <- 
    rdxklywind_hh |>
    as_tibble() |>
    mutate(datetime = time(rdxklywind_hh)) |>    
    relocate(datetime) |>
    slice(2:n())
...
p_sat <- calc_sat_water_vapour_press(wind$tem_avg)
